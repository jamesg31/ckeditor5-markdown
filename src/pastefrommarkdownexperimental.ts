import { Plugin, ClipboardPipeline, type ViewDocumentKeyDownEvent, type Editor, type ClipboardInputTransformationEvent } from 'ckeditor5';
...
export default class PasteFromMarkdownExperimental extends Plugin {
	 ...
	 * @internal
	 */
	private _mdDataProcessor: MDDataProcessor;
	...
	public static get pluginName() {
		return 'BetterPasteFromMarkdownExperimental' as const;
	}
	...
	public init(): void {
		...
		this.listenTo<ClipboardInputTransformationEvent>( clipboardPipeline, 'inputTransformation', ( evt, data ) => {
			...
			if ( !dataAsTextHtml ) {
				...
				data.content = this._mdDataProcessor.toView( dataAsTextPlain );
			...
			if ( markdownFromHtml ) {
				data.content = this._mdDataProcessor.toView( markdownFromHtml );
			}
		} );
	}
